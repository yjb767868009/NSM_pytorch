...
class RNN(nn.Module):
    def __init__(self, rnn_dims, rnn_activations, rnn_dropout):
        ...
        self.lstm5 = nn.LSTM(rnn_dims[6], rnn_dims[7], dropout=rnn_dropout, batch_first=True)
        self.lstm6 = nn.LSTM(rnn_dims[7], rnn_dims[8], dropout=rnn_dropout, batch_first=True)
        self.fc3 = nn.Sequential(nn.Dropout(rnn_dropout),
                                 nn.Linear(rnn_dims[8], rnn_dims[9]),
    ...
    def forward(self, x, x_length):
        ...
        x, (h_1, c_1) = self.lstm5(x)
        x, (h_1, c_1) = self.lstm6(x)
